...
if (!function_exists('relative_date')) {
    ...
    function relative_date($start_time) {
        ...
        if (date('Y', $now) == date('Y', $start_time)) {
            return mdate('Tháng %m', $start_time);
        } else {
            return mdate('Tháng %m, %Y', $start_time);
        }
    }
...
if (!function_exists('distant_time')) {
    ...
    function distant_time($time) {
        $seconds = time() - $time;
        if (($val = floor($seconds / 31536000)) > 0) {
            return $val . ' năm trước';
        }
        if (($val = floor($seconds / 2592000)) > 0) {
            return $val . ' tháng trước';
        }
        if (($val = floor($seconds / 86400)) > 0) {
            return $val . ' ngày trước';
        }
        if (($val = floor($seconds / 3600)) > 0) {
            return $val . ' giờ trước';
        }
        if (($val = floor($seconds / 60)) > 0) {
            return $val . ' phút trước';
        }
