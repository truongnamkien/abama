var show_alert = function(msg, options) {
    ...
    _default.html = '\
        <div class="lightbox ma20 blueColor">'
            ...
            + '<button class="button fRight mr10" id="btn_colorbox_close">' + _default.btnOk + '</button>'
            + '<div class="clear"></div>'
            + '</div>'
            + '</div>'
            + '<div class="lbPen"></div>'
...
var show_confirm = function(params) {
    ...
    var html = '\
        <div class="lightbox ma20 blueColor">'
            ...
            + '</div>'
            + '<div class="lbFooter mt10">'
            + '<button class="disabled_btn fRight mr10" id="confirm_btn_no">' + opts.btnCancel + '</button>'
            + '<button class="button fRight mr10" id="confirm_btn_yes">' + opts.btnOk + '</button>'
            + '<div class="clear"></div>'
            + '</div>'
            + '</div>'
            + '<div class="lbPen"></div>'
...
$(document).ready(function() {
    ...
    $("a.fake").live('click', function(e) {
        e.preventDefault();
    });

    $(".finish-thumb").hover(function(event) {
        var the_id = $(this).attr("id");
        var the_html = $("#" + the_id + "-hover").html();
        $("#hover-image").html(the_html).
                css({
                    "position": "absolute",
                    "left": $(this).position().left + 0,
                    "top": $(this).position().top + 70
                }).
                show();
    }, function(event) {
        var the_id = $(this).attr("id");
        $("#hover-image").html("").hide();
    });
